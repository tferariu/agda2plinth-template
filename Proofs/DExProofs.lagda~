...
mintingImpliesStart {record { sellCurr = sellC ; buyCurr = buyC }} adr oref tn top ctx@record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = (tok , lab) ; signature = signature ; continues = continues ; inputRef = inputRef ; mint = mint' ; tokCurrSymbol = cs } p1 p2 | False | True rewrite sym (==tto≡ (cs , tn) tok (get (get (go (consumes oref ctx) (go (continuingAddr adr ctx) p2)))))
  = TStart refl (==to≡ oref inputRef (get (go (continues) p2))) (go (ownAssetClass tn ctx == tok) (get (go (consumes oref ctx) (go (continuingAddr adr ctx) p2)))) , get p2 , ==ito≡ mint' (+ 1) eq' , go (checkDatum adr tn ctx) (go (consumes oref ctx) (go (continuingAddr adr ctx) p2))
...
startImpliesMinting {record { sellCurr = sellC ; buyCurr = buyC }} adr oref tn top ctx ((TStart refl refl p3) , refl , refl , p6 )
