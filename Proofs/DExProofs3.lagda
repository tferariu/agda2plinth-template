...
validatorImpliesRunning : ∀ {oref tn} (par : Params) (d : Datum) (i : Redeemer) (ctx : ScriptContext)
...
bothImplyFinal : ∀ (par : Params) (d : Datum) (adr : Address) (oref : TxOutRef) (tn : TokenName) (i : Redeemer) (ctx : ScriptContext)
               -> getMintedAmount ctx ≡ -1
               -> (agdaValidator par d i ctx && agdaPolicy adr oref tn tt ctx) ≡ true
               -> ( getPar par oref tn ⊢ getS d ctx ~[ i ]~| getS' d ctx
                  × continuing ctx ≡ false
...
transitionImpliesValidator par d (Update v r) ctx ((TUpdate refl refl refl p4 p5) , refl , p7 , p8 )
  rewrite p4 | p5 | n=n (owner (d .snd)) | v=v v | t=t (d .fst) | i=i (num r) | i=i (den r) | p7 | p8 = refl 
transitionImpliesValidator record { sellCurr = sellC ; buyCurr = buyC } d (Exchange amt pkh) ctx ((TExchange refl refl p3 p4) , refl , p6 , p7 )
  rewrite p3 | p4 | p6 | p7
    | v=v (newValue ctx + assetClassValue sellC amt)
    | t=t (d .fst) | i=i (num (ratio (d .snd))) | i=i (den (ratio (d .snd))) 
    | n=n (owner (snd d)) = refl

runningImpliesValidator : ∀ {oref tn} (par : Params) (d : Datum) (i : Redeemer) (ctx : ScriptContext)
                           -> (getPar par oref tn ⊢ getS d ctx ~[ i ]~> getS' d ctx
                              × continuing ctx ≡ true
                              × checkTokenIn (d .fst) ctx ≡ true
                              × checkTokenOut (d .fst) ctx ≡ true)
...
startImpliesMinting {record { sellCurr = sellC ; buyCurr = buyC }} adr oref tn top ctx ((TStart refl refl refl p4) , refl , refl , p7 )
  rewrite t=t (ownAssetClass tn ctx) | n=n oref | p4 | p7 = refl

initialImpliesMinting : ∀ (par : Params) (adr : Address) (oref : TxOutRef) (tn : TokenName) (top : ⊤) (ctx : ScriptContext)
                           -> ( getPar par oref tn ⊢ getMintS tn ctx
                              × continuing ctx ≡ true
                              × getMintedAmount ctx ≡ 1
                              × checkTokenOut (ownAssetClass tn ctx) ctx ≡ true)
                           -> agdaPolicy adr oref tn top ctx ≡ true
initialImpliesMinting record { sellCurr = sellC ; buyCurr = buyC } adr oref tn top ctx ((TStart refl refl refl p4) , refl , refl , p7 )
  rewrite t=t (ownAssetClass tn ctx) | n=n oref | p4 | p7 = refl
...
stopImpliesBoth : ∀ (par : Params) (d : Datum) (adr : Address) (oref : TxOutRef) (tn : TokenName) (i : Redeemer) (ctx : ScriptContext)
               -> (getPar par oref tn ⊢ getS d ctx ~[ i ]~| getS' d ctx
                  ...
                  × checkTokenIn (d .fst) ctx ≡ true)
               -> ((agdaValidator par d i ctx && agdaPolicy adr oref tn tt ctx) ≡ true)
stopImpliesBoth par d adr oref tn i ctx ((TStop refl) , refl , refl , p4 ) rewrite n=n (owner (d .snd)) | p4 = refl


finalImpliesBoth : ∀ {tn i} (par : Params) (d : Datum) (adr : Address) (oref : TxOutRef) (ctx : ScriptContext)
               -> (getPar par oref tn ⊢ getS d ctx ~[ i ]~| getS' d ctx
                  × continuing ctx ≡ false
                  × getMintedAmount ctx ≡ -1
                  × checkTokenIn (d .fst) ctx ≡ true)
...
Classifier record { par = par ; adr = adr ; oref = oref ; dat = dat ; red = Stop ; ctx = record { inputVal = inputVal ; outputVal = outputVal ; outputDatum = outputDatum ; signature = signature ; continues = continues ; inputRef = inputRef ; mint = (negsuc zero) } } = Final
