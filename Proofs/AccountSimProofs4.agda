...
module Proofs.AccountSimProofs4 where
...
bothImplyCleanup : ∀ {tn par} (d : Datum) (adr : Address) (oref : TxOutRef) (ctx : ScriptContext) 
                   ...
                   -> (par ⊢ getS d ctx ~[ Cleanup ]~| getS' ctx
                      × continuing ctx ≡ false
                      × checkTokenIn (d .fst) ctx ≡ true )
bothImplyCleanup d adr oref ctx refl pf = (TCleanup (==Lto≡ (snd d) [] (go (not (continuing ctx)) (go (checkTokenIn (d . fst) ctx) (get pf)))) refl , (unNot (get (go (checkTokenIn (d . fst) ctx) (get pf)))) , (get (get pf)))
