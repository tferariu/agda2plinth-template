...
module Proofs.AccountSimProofs2 where
...
initialFidelity : ∀ {s par}
  -> par ⊢ s
  -> Fides s
initialFidelity {record { datum = .(_ , []) ; value = value₁ ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} (TStart refl x₁ x₂ x₃ x₄ x₅ x₆) = ?

fidelity : ∀ {s s' : State} {i : Input} {par : MParams}
         -> Fides s
         -> par ⊢ s ~[ i ]~> s'
         -> Fides s'
...
initialValidity : ∀ {s par}
  -> par ⊢ s
  -> Valid s
initialValidity {record { datum = .(_ , []) ; value = emptyValue ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} (TStart refl p3 p4 p5 p6 p7 p8) = allNil , p4 , ?

validity : ∀ {s s' : State} {i par}
  -> Valid s
  -> par ⊢ s ~[ i ]~> s'
  -> Valid s'
...
validity {record { datum = tok , map ; value = value₁ ; tsig = tsig₁ ; continues = continues₁ ; spends = spends₁ ; hasToken = hasToken₁ ; mint = mint₁ ; token = token₁ }} {record { datum = .(_ , insert from (subValue vF val) (insert to (addValue vT val) map)) ; value = value₂ ; tsig = tsig₂ ; continues = continues₂ ; spends = spends₂ ; hasToken = hasToken₂ ; mint = mint₂ ; token = token₂ }} (fst , snd , thd) (TTransfer {par} {from} {to} {val} {vF = vF} {vT = vT} refl p2 p3 p4 p5 p6 p7 refl p9 p10 p11 p12 p13) = lem (insert to (addValue vT val) map) (subValue vF val) (diffLemma vF val p6 p5) (lem map (addValue vT val) (sumLemma vT val (geqLem map vT fst p4) p5) fst) , p11 , p13
...
stateInvariant : ∀ {s s' : State} {i par}
  -> Invariant s
  -> par ⊢ s ~[ i ]~> s'
  -> Invariant s'
...
getGeq refl (allCons {{i}} {{is}} , x , y) = i
...
liquidity s@record { datum = (tok , map) ; value = value ; tsig = tsig ; continues = continues ; spends = spends ; hasToken = hasToken ; mint = mint ; token = token } par (p@(p1 , p2 , p3) , p4) 
...
getPar : Argument -> MParams
getPar arg = record { address = arg .adr ; outputRef = arg .oref ; tokName = arg .tn}

-- The State Transition System as a relation
totalR : Argument -> Set
totalR arg with Classifier arg
... | Initial = getPar arg ⊢ getMintS (arg .tn) (arg .ctx)
... | Running = getPar arg ⊢ getS (arg .dat) (arg .ctx)  ~[ (arg .inp) ]~> getS' (arg .ctx) 
... | Terminal = getPar arg ⊢ getS (arg .dat) (arg .ctx)  ~[ (arg .inp) ]~| getS' (arg .ctx) 
