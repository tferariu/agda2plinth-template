...
-- Model State to be filled in with any contract-specific information
...
-- State Transition to be filled in according to the specification
...
-- Validity predicate dependant on the contract
Valid : State -> Set 
Valid s = {!!}
...
data _⊢_~[_]~*_ : MParams -> State -> List Input -> State -> Set where
  ...
  fin : ∀ { par s s' i is }
    -> par ⊢ s ~[ i ]~| s'  --I to be replaced with the specific input that closes the contract
    -------------------------
    -> par ⊢ s ~[ (i ∷ is) ]~* s'
...
--All of the equivalence proof components
...
-- The equivalence proof definitions
...
-- Defining which phase the model is in based on the argument
Classifier : Argument -> Phase
Classifier = {!!}

-- The validator and minting policy as a function
