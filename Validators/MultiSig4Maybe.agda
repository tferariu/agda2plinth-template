...
module Validators.MultiSig4Maybe where
...
agdaValidator param (tok , lab) red ctx = checkTokenIn tok ctx &&
  (case (lab , red) of λ where
    ...
    ((Collecting v pkh d sigs) , Pay) ->
      (length' sigs) >= (minSigs param) && continuing ctx && (checkTokenOut tok ctx) &&
      (case (newDatum ctx) of λ where
        (tok' , Holding) -> 
...
checkParams par
  = (noDups (par .authSigs)) && (length' (par .authSigs) >= par .minSigs) && par .maxWait > 0 
