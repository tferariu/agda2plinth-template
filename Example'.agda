...
example : Nat -> Nat -> Nat
example a b = if a >= 5 then a + b else b
...
checkWithdraw tok (Just v) pkh val map ctx = geq val emptyValue && geq v val && (newDatum ctx == (tok , insert pkh (v - val) map))

checkDeposit : AssetClass -> Maybe Value -> PubKeyHash -> Value -> AccMap -> ScriptContext -> Bool
checkDeposit tok Nothing _ _ _ _ = False
checkDeposit tok (Just v) pkh val map ctx = geq val emptyValue && (newDatum ctx == (tok , insert pkh (v + val) map))
...
checkTransfer tok (Just vF) (Just vT) from to val map ctx = geq val emptyValue && geq vF val && from /= to &&
...
agdaValidator (tok , map) inp ctx = checkTokenIn tok ctx && (case inp of λ where
    ...
    (Open pkh) -> checkTokenOut tok ctx && continuing ctx && checkSigned pkh ctx &&
                  not (checkMembership (lookup pkh map)) &&
                  newDatum ctx == (tok , insert pkh emptyValue map) && newValue ctx == oldValue ctx

    (Close pkh) -> checkTokenOut tok ctx && continuing ctx && checkSigned pkh ctx && checkEmpty (lookup pkh map) &&
                   newDatum ctx == (tok , delete pkh map) && newValue ctx == oldValue ctx
...
checkValue addr tn ctx = checkTokenOutAddr addr (ownAssetClass tn ctx) ctx &&
                         newValueAddr addr ctx == minValue + assetClassValue (ownAssetClass tn ctx) 1
